...
const ROLES_ACTIONS = (function() {
	var rolesActions = {};
	rolesActions[ROLE_UNKNOWN] = function(player, table, complete) {
		console.log('apply role: %s', ROLE_UNKNOWN);
		complete();
	};
	rolesActions[ROLE_THIEF] = function(player, table, complete) {
		console.log('apply role: %s', ROLE_THIEF);
		complete();
	};
	rolesActions[ROLE_CHEATER] = function(player, table, complete) {
		if (player.gold >= 10) {
			player.gold = 13;
		}
	...
	rolesActions[ROLE_WIDOW] = function(player, table, complete) {
		console.log('apply role: %s', ROLE_WIDOW);
		complete();
	};
	rolesActions[ROLE_PEASANT1] = function(player, table, complete) {
		console.log('apply role: %s', ROLE_PEASANT1);
		complete();
	};
	rolesActions[ROLE_PEASANT2] = rolesActions[ROLE_PEASANT1];

	rolesActions[ROLE_BEGGAR] = function(player, table, complete) {
		console.log('apply role: %s', ROLE_BEGGAR);
		complete();
	};
	rolesActions[ROLE_INQUISITOR] = function(player, table, complete) { 
		console.log('apply role: %s', ROLE_INQUISITOR);
		complete();
	};
	rolesActions[ROLE_SPY] = function(player, table, complete) {
		console.log('apply role: %s', ROLE_SPY);
		complete();
	};
	rolesActions[ROLE_JUDGE] = function(player, table, complete) {
		player.gold += table.tribunal;
		table.tribunal = 0;
		complete();
	};
	rolesActions[ROLE_KING] = function(player, table, complete) {
		player.gold += 3;
		complete();
	};
	rolesActions[ROLE_FOOL] = function(player, table, complete) {
		console.log('apply role: %s', ROLE_FOOL); 
		complete();
	};
	rolesActions[ROLE_BISHOP] = function(player, table, complete) {
		var minGold = player.gold;
		var stolenPlayer = null;

		table.forEach(function(otherPlayer) {
			if (otherPlayer.gold > minGold) { 
				stolenPlayer = otherPlayer;
			}
		});

		if (stolenPlayer) {
			var stolenGold = Math.min(stolenPlayer.gold, 2);
	...
	rolesActions[ROLE_QUEEN] = function(player, table, complete) {
		player.gold += 2;

		complete();
	};
	rolesActions[ROLE_WITCH] = function(player, table, complete) {

		var shouldTimeout = true;
		table.announceTurn.designatePlayersComplete = function(designedPlayers) {
			if (!(designedPlayers instanceof Array)) {
				table.sendTo(player, 'error', { type: 'designed-players-not-array' });
				return;
			}
			if (designedPlayers.length != 1) {
				table.sendTo(player, 'error', { type: 'designed-players-not-equal-1' });
				return;
			}

			var designedPlayerId = designedPlayers[0];
			if (designedPlayerId !== parseInt(designedPlayerId)) {
				table.sendTo(player, 'error', { type: 'designed-player-id-not-number' });
				return;
			}

			var designedPlayer = table.players.find(function(player) {
				return player.id == designedPlayerId;
			});

			if (!designedPlayer) {
				table.sendTo(player, 'error', { type: 'designed-player-not-found' });
				return;
			}

			var playerGold = player.gold;
			player.gold = designedPlayer.gold;
			designedPlayer.gold = playerGold;
		...
		table.sendTo(player, 'action:witch-designate', {});
		setTimeout(function() {
			if (shouldTimeout) {
				// TODO: error
				complete();
...
var Table = function() {
	...
	this.resolveAnnounceTurn = function() {
		...
		if (currentTable.announceTurn.samePlayers.length == 0 ||
			((currentTable.announceTurn.samePlayers.length == 1) && announcedRole == ROLE_PEASANT1)) {

			// don't need to reveal player

			// todo: handle second peasant
			ROLES_ACTIONS[announcedRole](currentPlayer, currentTable, function() {
			...
			// apply roles ...
			async.parallel(actualRoles.map(
				function(player) { 
					return function(cb) { ROLES_ACTIONS[announcedRole](player, currentTable, cb); } 
				}), 
				function(result) {
